...
def easycache_forward(
    ...
    pred_change = None
    raw_input_change_value = None
    output_norm_value = None
    accumulated_before = float(getattr(self, "accumulated_error", 0.0))
    accumulated_after = accumulated_before
    current_thresh = getattr(self, "thresh", None)
    region = "middle_high" if getattr(self, 'easycache_adaptive', False) else "fixed"
    if self.cnt < self.ret_steps or self.cnt >= self.num_steps - 1:
        should_calc = True
        self.accumulated_error = 0.0
        accumulated_after = 0.0
        region = "ret_full" if self.cnt < self.ret_steps else "final_full"
    else:
        if self.previous_raw_input is not None and self.previous_output is not None and self.k is not None:
            ...
            raw_input_change_value = raw_input_change.item()
            output_norm_value = output_norm.item()
            ...
            if getattr(self, 'easycache_adaptive', False):
                ...
                if self.cnt <= fs or self.cnt >= self.num_steps - ls - 1:
                    ...
                    region = "early_low" if self.cnt <= fs else "late_low"
                else:
                    current_thresh = self.thresh_high
                    region = "middle_high"

            force_boundary_full = (
                getattr(self, "boundary_full", False)
                and getattr(self, "easycache_adaptive", False)
                and (
                    self.cnt == getattr(self, "first_steps", 8) + 1
                    or self.cnt == self.num_steps - getattr(self, "last_steps", 6) - 1
                )
            )
            if force_boundary_full:
                should_calc = True
                self.accumulated_error = 0.0
                accumulated_after = 0.0
                region = f"{region}_boundary_full"
            elif self.accumulated_error < current_thresh:
                should_calc = False
                accumulated_after = float(self.accumulated_error)
            else:
                should_calc = True
                self.accumulated_error = 0.0
                accumulated_after = 0.0
    ...
    self.skip_streak = getattr(self, "skip_streak", 0) + (0 if should_calc else 1)
    if should_calc:
        self.skip_streak = 0
    if hasattr(self, "decision_history"):
        self.decision_history.append({
            "step": step_idx,
            "region": region,
            "threshold": None if current_thresh is None else float(current_thresh),
            "pred_change": pred_change,
            "accumulated_before": accumulated_before,
            "accumulated_after": accumulated_after,
            "should_calc": bool(should_calc),
            "action": "full" if should_calc else "skip",
            "skip_streak": int(self.skip_streak),
            "raw_input_change": raw_input_change_value,
            "output_norm": output_norm_value,
            "k": None if getattr(self, "k", None) is None else float(self.k.item()),
        })
...
def easycache_baseline_forward(
    ...
    if self.previous_raw_input is not None and self.previous_output is not None and self.k is not None:
        ...
        if hasattr(self, "decision_history"):
            self.decision_history.append({
                "step": step_idx,
                "region": "baseline_full",
                "threshold": None,
                "pred_change": pred_change,
                "accumulated_before": None,
                "accumulated_after": None,
                "should_calc": True,
                "action": "full",
                "skip_streak": 0,
                "raw_input_change": raw_input_change.item(),
                "output_norm": output_norm.item(),
                "k": float(self.k.item()),
            })
    elif hasattr(self, "decision_history"):
        self.decision_history.append({
            "step": step_idx,
            "region": "baseline_warmup",
            "threshold": None,
            "pred_change": None,
            "accumulated_before": None,
            "accumulated_after": None,
...
def main():
    ...
    exp_parser.add_argument(
        "--easycache-boundary-full",
        action="store_true",
        help="In adaptive mode, force a full step at low/high threshold boundaries.",
    )
    ...
    transformer.__class__.decision_history = []
    transformer.__class__.skip_streak = 0
    ...
    if exp_args.easycache_mode in ("easycache", "adaptive"):
        ...
        if exp_args.easycache_mode == "adaptive":
            ...
            transformer.__class__.boundary_full = exp_args.easycache_boundary_full
            boundary_tag = "_boundary_full" if exp_args.easycache_boundary_full else ""
            mode_tag = (
                f"easycache_adaptive_l{exp_args.easycache_thresh_low}_"
                f"h{exp_args.easycache_thresh_high}_"
                f"f{exp_args.easycache_first_steps}l{exp_args.easycache_last_steps}"
                f"{boundary_tag}"
            )
        else:
    ...
    decision_history = getattr(transformer, "decision_history", [])
    if len(decision_history) > 0:
        decision_json_path = os.path.join(save_path, "decision_trace.json")
        with open(decision_json_path, "w") as f:
            json.dump(decision_history, f, indent=2)
        logger.info(f"Decision trace JSON saved to: {decision_json_path}")

        decision_csv_path = os.path.join(save_path, "decision_trace.csv")
        with open(decision_csv_path, "w", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=list(decision_history[0].keys()))
            writer.writeheader()
            writer.writerows(decision_history)
        logger.info(f"Decision trace CSV saved to: {decision_csv_path}")

        steps_trace = [d["step"] for d in decision_history]
        pred_trace = [
            np.nan if d["pred_change"] is None else d["pred_change"]
            for d in decision_history
        ]
        acc_trace = [
            np.nan if d["accumulated_after"] is None else d["accumulated_after"]
            for d in decision_history
        ]
        full_steps = [d["step"] for d in decision_history if d["action"] == "full"]
        skip_steps = [d["step"] for d in decision_history if d["action"] == "skip"]

        fig, axes = plt.subplots(3, 1, figsize=(12, 9), sharex=True)
        axes[0].plot(steps_trace, pred_trace, "g-", marker="o", markersize=4)
        axes[0].set_ylabel("pred_change")
        axes[0].grid(True, alpha=0.3)

        axes[1].plot(steps_trace, acc_trace, "r-", marker="^", markersize=4)
        axes[1].set_ylabel("accumulated")
        axes[1].grid(True, alpha=0.3)

        axes[2].scatter(full_steps, [1] * len(full_steps), color="#2ecc71", label="full", s=50)
        axes[2].scatter(skip_steps, [0] * len(skip_steps), color="#e74c3c", label="skip", s=50)
        axes[2].set_yticks([0, 1])
        axes[2].set_yticklabels(["skip", "full"])
        axes[2].set_xlabel("Diffusion Step")
        axes[2].set_ylabel("action")
        axes[2].legend(loc="best")
        axes[2].grid(True, alpha=0.3)

        fig.suptitle(f"EasyCache Decision Trace: {exp_args.easycache_mode}")
        decision_plot_path = os.path.join(save_path, "decision_trace_plot.png")
        fig.savefig(decision_plot_path, dpi=300, bbox_inches="tight")
        plt.close(fig)
        logger.info(f"Decision trace plot saved to: {decision_plot_path}")
