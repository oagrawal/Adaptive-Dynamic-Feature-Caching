...
# Data - using speedup values provided by user
speedups = {
...
for filename in os.listdir(fidelity_dir):
    if filename.endswith(".json"):
        # We need the mode results which compare to baseline
        # These files typically contain the mode name inside the JSON
...
for metric in metrics:
    ...
    plt.title(f"Wan2.1 DiCache Pareto: {metric.upper()} vs Speedup (Top-Right is Better)", fontsize=14, pad=20)
    ...
    output_path = f"/nfs/oagrawal/wan/Wan2.1/dicache_results/pareto_frontier_wan_{metric}_v2.png"
