...
def dicache_forward(
    ...
    if self.cnt >= int(self.num_steps * self.ret_ratio) and self.previous_input[self.cnt%2] is not None:
        ...
        thresh = self.rel_l1_thresh
        if self.adaptive:
            thresh = self.thresh_low if (self.cnt < self.stable_start or self.cnt >= self.stable_end) else self.thresh_high
        if self.accumulated_rel_l1_distance[self.cnt%2] < thresh: # skip this step
            skip_forward = True
...
def _parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Use adaptive thresholds (--thresh_low for early/late, --thresh_high for stable middle).")
    parser.add_argument(
        "--thresh_high",
        type=float,
        default=0.20,
        help="Threshold for stable middle region when --adaptive is set.")
    parser.add_argument(
        "--thresh_low",
        type=float,
        default=0.05,
        help="Threshold for volatile early/late region when --adaptive is set.")
    parser.add_argument(
        "--stable_start",
        type=int,
        default=16,
        help="First cnt index that enters the stable (high-threshold) region.")
    parser.add_argument(
        "--stable_end",
        type=int,
        default=70,
        help="First cnt index that returns to the volatile (low-threshold) region.")
    parser.add_argument(
        "--calibrate",
        action="store_true",
        default=False,
...
def generate(args):
    ...
    if not args.baseline:
        ...
        wan_t2v.model.__class__.adaptive = args.adaptive
        wan_t2v.model.__class__.thresh_high = args.thresh_high
        wan_t2v.model.__class__.thresh_low = args.thresh_low
        wan_t2v.model.__class__.stable_start = args.stable_start
        wan_t2v.model.__class__.stable_end = args.stable_end
